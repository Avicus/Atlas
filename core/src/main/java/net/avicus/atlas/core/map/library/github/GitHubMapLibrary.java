package net.avicus.atlas.core.map.library.github;
...
import net.avicus.atlas.core.match.MatchBuildException;
import net.avicus.atlas.core.util.xml.XmlException;
import org.javalite.http.Get;
import org.javalite.http.Http;

/**
 * A library of maps that is stored inside of a github repository. This does not require git to be
 * installed.
 */
@ToString
public class GitHubMapLibrary implements MapLibrary {
      ...
      if (file.getName().equals("map.xml")) {
        ...
        for (GithubItem item : items) {
          ...
          String path = item.getPath()
              .substring(index + folderName.length() + 1);
  ...
  /**
   * Get a {@link JsonArray} from the github API. If authentication is configured in the config, it
   * will be used.
   *
