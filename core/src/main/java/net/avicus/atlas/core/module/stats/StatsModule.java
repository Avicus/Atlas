...
import net.avicus.atlas.core.event.player.PlayerReceiveMVPEvent;
import net.avicus.atlas.core.match.Match;
import net.avicus.atlas.core.module.BridgeableModule;
import net.avicus.atlas.core.module.Module;
import net.avicus.atlas.core.module.ModuleBridge;
import net.avicus.atlas.core.module.stats.action.ActionStore;
import net.avicus.atlas.core.module.stats.action.lifetime.LifetimeDisplayUtils;
    ...
    if (event.isChangeToNotPlaying() && event.isFromPlaying()) {
      ...
      AtlasTask.of(() -> {
        if (this.match.isLoaded()) {
          ...
          Player mvpPlayer = Bukkit.getPlayer(mvp);
          if (mvpPlayer == null) {
            return;
          }
          Localizable name = Messages.UI_MVP_TITLE
              .with(ChatColor.GREEN, Users.getLocalizedDisplay(mvpPlayer, false));
          Localizable sub;
          if (!mvpPlayer.isOnline()) {
          ...
          Events.call(new PlayerReceiveMVPEvent(mvpPlayer));
          ...
          data.append("MVP: " + mvpPlayer.getName() + "\n");
          ...
          StaffChannels.DEV_CHANNEL.send(null, message);
        }
      }).laterAsync(5 * 20);
    }
  }
