...
import net.avicus.atlas.core.event.world.EntityChangeEvent;
...
public class ObserverListener implements Listener {
  ...
  public void onInventoryClick(InventoryClickEvent event) {
    if (event.getWhoClicked() instanceof Player && notPlaying(event.getWhoClicked())) {
      if (event.getInventory().getType() != InventoryType.PLAYER && !(event.getInventory()
          .getHolder() instanceof InventoryMenuAdapter)) {
        event.setCancelled(true);
      }
    }
  }
