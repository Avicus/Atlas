package net.avicus.atlas.core.component.freeze;
...
public class FreezeComponent implements CommandComponent, ListenerComponent {
  ...
  private void extinguish(final Player victim) {
    final int tntRadius = AtlasConfig.Freeze.Radius.getTnt();
    if (tntRadius != -1) {
    ...
    final int extinguishRadius = AtlasConfig.Freeze.Radius.getExtinguish();
  ...
  public static class Commands {
    ...
    public static void freeze(CommandContext context, CommandSender sender) {
      ...
      Atlas.get().getComponentManager().get(FreezeComponent.class).freezeCommand(sender, target);
    }
  }
}
